			     ...
			     struct drm_plane_state *plane_state)
{
	...
	pr_info("[ILI9486] Enter waveshare_enable: g_bgr_enabled = %d\n", g_bgr_enabled);
	...
	msleep(100);
    // mipi_dbi_command(dbi, MIPI_DCS_SET_DISPLAY_INVERSION_MODE, 0x00);
 ...
 out_enable:
	...
	// 根据g_bgr_enabled设置BGR位
	if (g_bgr_enabled) {
		addr_mode |= ILI9486_MADCTL_BGR;
		pr_info("[ILI9486] BGR enabled: addr_mode = 0x%02X\n", addr_mode);
	} else {
		addr_mode &= ~ILI9486_MADCTL_BGR;  // 确保禁用BGR（清除第3位）
		pr_info("[ILI9486] BGR disabled: addr_mode = 0x%02X\n", addr_mode);
	}
...
static int ili9486_probe(struct spi_device *spi)
{
	...
	pr_info("[ILI9486] Probe: Read bgr property from device tree → g_bgr_enabled = %d\n", g_bgr_enabled);
    ...
    dbi->swap_bytes = false;  // 新增：强制禁用像素字节交换（关键修复）
